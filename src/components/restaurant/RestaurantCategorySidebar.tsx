...
/** The sticky "ប្រភេទ" (categories) rail from the reference design.
 *  Desktop: sticky column, each row shows an icon, label and a `>` chevron;
 *  the active row is pale-green with green text.
 *  Tablet/mobile: collapses into a horizontal, swipeable chip row. */
export default function RestaurantCategorySidebar({ categories }: Props) {
  const [activeKey, setActiveKey] = useState(categories[0]?.key);
            ...
            return (
              <li key={category.key}>
                ...
                >
                  <CategoryIcon
                    icon={category.icon}
                    className="text-base shrink-0 text-primary-700"
                  />
                  <span className="line-clamp-1 flex-1">{category.label}</span>
                  <FaChevronRight
                    className={`shrink-0 text-xs ${
                      active ? "text-primary-700" : "text-gray-300"
                    }`}
                  />
                </button>
              </li>
      ...
      >
        {categories.map((category) => {
          ...
          return (
            ...
            >
              <CategoryIcon icon={category.icon} className="text-sm text-primary-700" />
              {category.label}
            </button>
          );
        })}
