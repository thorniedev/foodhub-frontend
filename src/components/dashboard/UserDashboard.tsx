...
function calculateMemberSinceDays(createdAt?: string | null): number {
...
export default function UserDashboard() {
  ...
  const defaultProfileSummary = useMemo(
    () =>
      ...
      (profilesResponse?.contents && profilesResponse.contents.length > 0
        ? profilesResponse.contents[0]
        : null),
    [profilesResponse],
  );
  ...
  const otherProfiles = useMemo(
    () =>
      profilesResponse?.contents.filter(
        (memberProfile) => memberProfile.uuid !== defaultProfileSummary?.uuid,
      ) ?? [],
    [profilesResponse, defaultProfileSummary?.uuid],
  );
