    ...
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-20">
      ...
      <motion.div
        ...
        animate={
          reduceMotion
            ? undefined
            : { x: [0, 65, 0], y: [0, 30, 0], scale: [1, 1.14, 1] }
        }
      ...
      {PARTICLES.map((particle, index) => (
        <motion.span
          ...
          className={`absolute rounded-full ${
            index % 4 === 0
              ? "bg-orange-400/[0.65] shadow-[0_0_18px_rgba(251,146,60,0.65)]"
              : "bg-emerald-400/[0.60] shadow-[0_0_18px_rgba(52,211,153,0.58)]"
          }`}
          style={{
            left: particle.left,
            top: particle.top,
        ...
        return (
          <g key={feature.key}>
            <path
              ...
              className="text-slate-300/[0.45] dark:text-white/10"
            />
            ...
            <motion.path
              ...
              animate={
                reduceMotion
                  ...
                  : {
                      pathLength: active ? [0.15, 1, 0.15] : 1,
                      pathOffset: active ? [0, 0.06, 0] : 0,
                      opacity: active ? [0.4, 1, 0.4] : 0.14,
                    }
              }
              transition={{
                duration: active ? 1.9 : 0.35,
  ...
  return (
    <div className={`absolute z-30 ${feature.positionClassName}`}>
      <motion.button
        ...
        aria-pressed={active}
        onPointerEnter={() => onActivate(feature.key)}
        onFocus={() => onActivate(feature.key)}
        onClick={() => onActivate(feature.key)}
        whileHover={reduceMotion ? undefined : { scale: 1.08, y: -7 }}
        whileTap={{ scale: 0.95 }}
        animate={
          reduceMotion
            ? undefined
        ...
        className={`group relative flex h-14 min-w-14 items-center justify-center gap-3 overflow-hidden rounded-2xl border px-2.5 backdrop-blur-2xl transition-[border-color,background-color,color,box-shadow] duration-300 sm:h-16 sm:min-w-[150px] sm:justify-start sm:px-3.5 ${
          active
            ? emerald
              ? "border-emerald-300/90 bg-white/[0.92] text-emerald-700 shadow-[0_18px_50px_-18px_rgba(16,185,129,0.78)] dark:border-emerald-300/70 dark:bg-emerald-950/[0.65] dark:text-emerald-100"
              : "border-orange-300/90 bg-white/[0.92] text-orange-700 shadow-[0_18px_50px_-18px_rgba(249,115,22,0.76)] dark:border-orange-300/70 dark:bg-orange-950/[0.55] dark:text-orange-100"
            : "border-white/80 bg-white/[0.72] text-slate-700 shadow-[0_16px_44px_-24px_rgba(15,23,42,0.65)] hover:border-emerald-300 dark:border-white/10 dark:bg-slate-950/[0.58] dark:text-slate-200"
        }`}
      >
        {active && (
          <motion.span
        ...
        {active && !reduceMotion && (
          <motion.span
            aria-hidden
            className="absolute -left-14 top-0 h-full w-10 skew-x-[-18deg] bg-white/[0.85] blur-sm"
            animate={{ left: ["-30%", "130%"] }}
            transition={{
              duration: 1.4,
              repeat: Infinity,
        ...
        <motion.span
          ...
          animate={
            active && !reduceMotion
              ? { scale: [0.7, 1.4, 0.7], opacity: [0.6, 1, 0.6] }
              : undefined
          }
    ...
    <div className="absolute inset-0 grid place-items-center">
      ...
      >
        ...
        <div className="relative flex size-full flex-col items-center justify-center overflow-hidden rounded-[2.35rem] border border-white/90 bg-white/90 p-3 backdrop-blur-2xl sm:rounded-[2.95rem] sm:p-5 dark:border-white/10 dark:bg-[#06130e]/90">
          ...
          {/* Moving scanner across the interface */}
          {!reduceMotion && (
            <motion.span
              aria-hidden
          ...
          >
            {/* AI Core */}
            <div className="relative flex size-[84px] items-center justify-center sm:size-[124px]">
              {/* Energy pulse */}
              <motion.span
                aria-hidden
              ...
              <motion.span
                aria-hidden
                className={`absolute -bottom-1 -right-1 z-30 flex size-8 items-center justify-center rounded-xl border-2 border-white text-white shadow-lg sm:size-10 ${
...
export default function FoodHubHero01({
  logoSrc = "/Image/foodHub-logo.webp",
  ...
  const glowX = useSpring(glowXInput, { stiffness: 80, damping: 22 });
  const glowY = useSpring(glowYInput, { stiffness: 80, damping: 22 });
  const layerX = useSpring(layerXInput, { stiffness: 75, damping: 24 });
  const layerY = useSpring(layerYInput, { stiffness: 75, damping: 24 });
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      setActiveKey((currentKey) => {
        ...
        return FEATURES[(currentIndex + 1) % FEATURES.length].key;
      });
    }, 2300);
    ...
    >
      ...
      {!reduceMotion && (
        <motion.div
          ...
          className="pointer-events-none absolute left-1/2 top-[62%] -z-10 size-[420px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-300/[0.18] blur-[110px] sm:size-[560px] dark:bg-emerald-400/10"
        />
      )}
      ...
      <div className="container mx-auto flex min-h-screen max-w-7xl flex-col items-center justify-center px-4 py-14 sm:px-6 sm:py-16 lg:px-0 lg:py-8">
        ...
        <motion.div
          initial={
            reduceMotion
              ? false
              : { opacity: 0, scale: 0.9, y: 35, filter: "blur(14px)" }
          }
