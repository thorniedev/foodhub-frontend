...
import { Suspense, useEffect, useMemo, useState, useRef, memo, type ReactNode } from "react";
...
type FilterSidebarProps = {
  ...
  mobile?: boolean;
  onClose?: () => void;
  initialFilters?: DiscoveryFilterOptionsResponse | null;
...
const FilterSidebar = memo(function FilterSidebar({
  ...
  memberProfiles: passedMemberProfiles = [],
}: FilterSidebarProps) {
  const { data: fetchedFilters } = useGetDiscoveryFiltersQuery();
  const filterOptions = fetchedFilters || initialFilters || undefined;
  const { data: fetchedMenuItems = [] } = useGetMenuItemsQuery(undefined, {
    skip: passedMenuItems.length > 0,
  });
  const menuItems = passedMenuItems.length > 0 ? passedMenuItems : fetchedMenuItems;
  const { data: profileResponse } = useGetMemberProfilesQuery(undefined, {
    skip: passedMemberProfiles.length > 0,
  });
  const { data: foodCatalogList = [] } = useGetFoodCatalogListQuery(undefined, {
    skip: menuItems.length > 0 && Boolean(filterOptions),
  });

  // ✅ PERFORMANCE FIX: Use catalog menuItems directly — eliminates 60 parallel
  // getMenuItemByUuid network calls that previously choked the browser event loop.
  const activeMenuItems = menuItems;

  const memberProfiles =
    passedMemberProfiles.length > 0
      ? passedMemberProfiles
  ...
  /*
   * Auto-expand active filter sections. openSections is still real state (the
   * user can collapse a section by hand), so this adjusts it while rendering
   * rather than from an effect -- the effect version re-rendered this whole
   * tree again after every filter change just to open an accordion.
   */
  const sectionsToAutoOpen = useMemo(() => {
    const next: Record<string, boolean> = {};
    if (customerSearchRequest.ageGroupUuids?.length) next.ageGroup = true;
    if (customerSearchRequest.provinces?.length) next.province = true;
    if (customerSearchRequest.seasonUuids?.length) next.season = true;
    if (customerSearchRequest.eventUuids?.length) next.event = true;
    if (customerSearchRequest.cuisineUuids?.length) next.cuisine = true;
    if (customerSearchRequest.dietaryTypeUuids?.length) next.dietary = true;
    if (customerSearchRequest.excludeAllergenUuids?.length)
      next.allergens = true;
    if (customerSearchRequest.weatherConditionUuids?.length)
      next.weather = true;
    return next;
  }, [
  ...
  const autoOpenSignature = Object.keys(sectionsToAutoOpen).sort().join(",");
  const [prevAutoOpenSignature, setPrevAutoOpenSignature] =
    useState(autoOpenSignature);

  if (autoOpenSignature !== prevAutoOpenSignature) {
    setPrevAutoOpenSignature(autoOpenSignature);

    if (autoOpenSignature) {
      setOpenSections((previous) => {
        // Don't churn state if every section is already open.
        const needsChange = Object.keys(sectionsToAutoOpen).some(
          (key) => !previous[key],
        );
        return needsChange
          ? { ...previous, ...sectionsToAutoOpen }
          : previous;
      });
    }
  }
...
const FoodGrid = memo(function FoodGrid({ foods, isLoading }: FoodGridProps) {
  ...
  return (
    ...
    >
      {foods.map((food, index) => (
        <div key={food.uuid} className="w-full">
          {/* FoodCard already contains its own Link */}
          <FoodCard food={food} priority={index < 6} />
        </div>
      ))}
    </div>
  );
...
export type MenuPageClientProps = {
  initialMenuItems?: CatalogMenuItem[];
  initialFilters?: DiscoveryFilterOptionsResponse | null;
...
function FoodPageContent({
  ...
  const urlParams = searchParamsProp;

  const { coordinates: userCoordinates } = useUserLocation();
  const rawAgeParam = getParam(urlParams, "ageGroups", "ageGroup", "age");
  const rawProvinceParam = getParam(
    urlParams,
    "province",
    "provinces",
    "location",
  ...
  const rawSeasonParam = getParam(urlParams, "season", "seasons");
  const rawEventParam = getParam(urlParams, "event", "events");
  const rawCategoryParam = getParam(urlParams, "category", "categories");
  const rawCuisineParam = getParam(urlParams, "cuisine", "cuisines");
  const rawQueryParam = getParam(urlParams, "query", "q", "search");
  ...
  const isUuid = (val?: string | null): val is string =>
    Boolean(val && UUID_REGEX.test(val.trim()));
  ...
  const discoveryFilterOptions =
    fetchedFilters || initialFilters || undefined;
  const { data: foodCatalogList = [] } = useGetFoodCatalogListQuery();

  /*
   * Every URL-derived filter is resolved in ONE pass during render instead of
   * in seven separate effects (age / province / season / event / category /
   * cuisine / query). Each of those used to setState from an effect, so a
   * single /menu?category=..&cuisine=.. load queued a chain of extra render
   * passes through this whole tree after the first paint. Deriving it here and
   * applying it with the documented "adjust state while rendering" escape
   * hatch below collapses that into one pre-paint adjustment:
   * https://react.dev/learn/you-might-not-need-an-effect#adjusting-some-state-when-a-prop-changes
   */
  const urlDerivedFilters = useMemo(() => {
    const search: Partial<CustomerSearchRequest> = {};
    let ageGroupCodes: string[] | undefined;
    ...
    /* ---- age group (also drives the visible filter chips) ---- */
    const decodedAge = decodeParam(rawAgeParam);
    if (decodedAge) {
      const ageGroups = discoveryFilterOptions?.ageGroups;

      if (decodedAge === "ALL" && ageGroups && ageGroups.length > 0) {
        search.ageGroupUuids = ageGroups.map((ag) => ag.uuid);
        ageGroupCodes = ageGroups.map((ag) => ag.code || ag.name);
      } else {
        const matched =
          ageGroups && ageGroups.length > 0
            ? findMatchingAgeGroup(decodedAge, ageGroups)
            : undefined;

        if (matched) {
          search.ageGroupUuids = [matched.uuid];
          ageGroupCodes = [matched.code || matched.name];
        } else {
          // Fallback: only a real UUID is safe to send to the backend.
          if (isUuid(decodedAge)) {
            search.ageGroupUuids = [decodedAge];
          }
          ageGroupCodes = [decodedAge];
        }
      }
    }

    /* ---- province (token match, falls back to the raw value) ---- */
    const decodedProvince = decodeParam(rawProvinceParam);
    if (decodedProvince) {
      const provTokens = getProvinceSearchTokens(decodedProvince);
      const provinces = discoveryFilterOptions?.provinces;
      const found =
      ...
      search.provinces = [found ?? decodedProvince];
    }

    /*
     * season / event / category / cuisine were four byte-identical effects
     * apart from the option list and the field they wrote to.
     */
    const matchByNameCodeOrUuid = <
      T extends { uuid: string; name?: string; code?: string },
    >(
      decoded: string,
      options?: T[],
    ): T | undefined => {
      ...
      return options.find(
        (o) =>
          normalizeText(o.name) === normalizeText(decoded) ||
          normalizeText(o.code) === normalizeText(decoded) ||
          normalizeText(o.uuid) === normalizeText(decoded),
      );
    };
    ...
    const simpleParams = [
      {
        raw: rawSeasonParam,
        options: discoveryFilterOptions?.seasons,
        field: "seasonUuids",
      },
      {
        raw: rawEventParam,
        options: discoveryFilterOptions?.events,
        field: "eventUuids",
      },
      {
        raw: rawCategoryParam,
        options: discoveryFilterOptions?.categories,
        field: "categoryUuids",
      },
      {
        raw: rawCuisineParam,
        options: discoveryFilterOptions?.cuisines,
        field: "cuisineUuids",
      },
    ] as const;

    for (const { raw, options, field } of simpleParams) {
      const decoded = decodeParam(raw);
      if (!decoded) continue;

      const matched = matchByNameCodeOrUuid(decoded, options);

      if (matched) {
        search[field] = [matched.uuid];
      } else if (isUuid(decoded)) {
        search[field] = [decoded];
      }
    }

    return { search, ageGroupCodes };
  }, [
    rawAgeParam,
    rawProvinceParam,
    rawSeasonParam,
  ...
  /*
   * Applied during render rather than in an effect. The signature keeps this
   * from re-firing when useMemo recomputes to the same values (e.g. the
   * filter-options query refetching and handing back a new object identity).
   */
  const urlFilterSignature = JSON.stringify([
    urlDerivedFilters.search,
    urlDerivedFilters.ageGroupCodes ?? null,
    rawQueryParam,
  ]);
  const [prevUrlFilterSignature, setPrevUrlFilterSignature] =
    useState(urlFilterSignature);

  if (urlFilterSignature !== prevUrlFilterSignature) {
    setPrevUrlFilterSignature(urlFilterSignature);

    const { search, ageGroupCodes } = urlDerivedFilters;

    if (Object.keys(search).length > 0) {
      setCustomerSearchRequest((prev) => {
        // Preserve the original bail-out: don't churn state when the URL
        // value is already the one applied.
        const alreadyApplied = (
          Object.keys(search) as (keyof CustomerSearchRequest)[]
        ).every((key) => {
          ...
          return (
            ...
            next.every((v) => (current as unknown[]).includes(v))
          );
        });

        return alreadyApplied ? prev : { ...prev, ...search };
      });
    }

    if (ageGroupCodes) {
      setFilters((prev) => ({ ...prev, ageGroupCodes }));
    }
  ...
  const hasUserSearchedOrFiltered = Boolean(
    searchInput.trim() ||
      customerSearchRequest.categoryUuids?.length ||
      customerSearchRequest.cuisineUuids?.length ||
      customerSearchRequest.mealTypeUuids?.length ||
      customerSearchRequest.ageGroupUuids?.length ||
      customerSearchRequest.seasonUuids?.length ||
      customerSearchRequest.eventUuids?.length ||
      customerSearchRequest.weatherConditionUuids?.length ||
      customerSearchRequest.dietaryTypeUuids?.length ||
      customerSearchRequest.excludeAllergenUuids?.length ||
      customerSearchRequest.storePriceLevels?.length ||
      customerSearchRequest.availabilityStatuses?.length ||
      customerSearchRequest.provinces?.length ||
      customerSearchRequest.cities?.length ||
      customerSearchRequest.featuredOnly ||
      customerSearchRequest.minimumStoreRating !== undefined ||
      customerSearchRequest.openNow ||
      customerSearchRequest.minimumPrice !== undefined ||
      customerSearchRequest.maximumPrice !== undefined ||
      customerSearchRequest.minimumSpiceLevel !== undefined ||
      customerSearchRequest.maximumSpiceLevel !== undefined ||
      customerSearchRequest.maxPreparationTimeMinutes !== undefined ||
      customerSearchRequest.profileUuid
  );

  useEffect(() => {
    if (!hasUserSearchedOrFiltered) return;
    executeDiscoverySearch({
      page: 0,
      size: 50,
    ...
    setCurrentPage(1);
  }, [customerSearchRequest, searchInput, userCoordinates, executeDiscoverySearch, hasUserSearchedOrFiltered]);
  ...
  const {
    ...
    skip: Boolean(initialMenuItems && initialMenuItems.length > 0),
  });

  const menuItems =
    fetchedMenuItems && fetchedMenuItems.length > 0
      ? fetchedMenuItems
      : initialMenuItems;
  ...
  const renderSearch = () => (
    <div className="relative flex min-h-[56px] w-full flex-1 items-center gap-3 rounded-2xl lg:rounded-full border border-gray-200/80 bg-white px-4 shadow-sm transition-all focus-within:border-primary-700 focus-within:ring-2 focus-within:ring-primary-700/20 dark:border-slate-800 dark:bg-slate-900">
      ...
      <input
        ...
        aria-label="ស្វែងរកមុខម្ហូប ឈ្មោះ កូដ ប្រភេទ ហាង"
